...
(function ($) {
    ...
    $.fn.ConnectionsManagerCreate = function () {
        var connStringHolder = 'DATA SOURCE={DataSource};PASSWORD={Password};PERSIST SECURITY INFO=True;USER ID={UserId}';
        var validConnection = false;

        $('.checkCString').on('click', function () {
            var me = $(this);
            var Name = $('#Name').val();
            ...
            var cstring = connStringHolder.format({ DataSource: DataSource, UserId: UserId, Password: Password });
            var pResult = connStringHolder.format({ DataSource: DataSource, UserId: UserId, Password: iPassword });


            $.get(
                me.attr('href'),
                { ConnectionString: pResult },
                function (result) {
                    alert(result);
                    if (result == true) {
                        ...
                        cstring += '\nNo pudo realizarse la conexión con la base de datos.';
                        validConnection = true;
                    }
                    $('#CstringResult').val(cstring);
                }
            );
